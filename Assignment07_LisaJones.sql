...
-- 2017-01-01,LisaJones,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_LisaJones')
	 Begin 
	  Alter Database [Assignment07DB_LisaJones] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_LisaJones;
	 End
	Create Database Assignment07DB_LisaJones;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_LisaJones;
...
go
Select ProductName, Format(UnitPrice, 'C') as 'US Format'
	From vProducts
...
go
Select c.CategoryName, p.ProductName, Format(UnitPrice, 'C') as 'US Format'
...
AS
	Select TOP 100 Percent
		p.ProductName, (DateName(mm, i.InventoryDate) + ', ' + DateName(YYYY, i.InventoryDate)) as InventoryDate, i.Count as InventoryCount
...
AS
	Select TOP 100 Percent
		c.CategoryName, (DateName(mm, i.InventoryDate) + ', ' + DateName(YYYY, i.InventoryDate)) as InventoryDate, Sum([Count]) as InventoryCountByCategory
